...
function initVizPlayground(){
  ...
  function trainEpochs(steps){
    ...
    for(var e=0;e<steps;e++){
      ...
      if(e===steps-1)$("pg-calc").textContent="Epoch "+epoch+" • Loss "+loss.toFixed(4)+" (0 = sempurna) • Akurasi "+(acc*100).toFixed(1)+"%"+(W.h1===0?"\n⚠️ Hidden layer 0 = tanpa lapisan tengah, cuma garis lurus — mustahil untuk lingkaran/spiral! Tambah neuronnya.":"");
    }
  ...
  function drawLoss(){
    ...
    lctx.fillStyle="#64748b";lctx.font="11px sans-serif";lctx.fillText("grafik loss ("+lossHist.length+" titik)",10,14);
  }
  ...
  $("pg-reset").onclick=function(){stop();initW();drawBoundary();drawLoss();$("pg-calc").textContent="Bobot diacak ulang. Klik Latih!"};
  $("pg-newdata").onclick=function(){stop();genData();initW();drawBoundary();drawLoss();$("pg-calc").textContent="Dataset baru datang! Klik Latih."};
["pg-data","pg-noise"].forEach(function(id){$(id).addEventListener("change",function(){stop();genData();initW();drawBoundary();drawLoss();$("pg-calc").textContent="Dataset diganti. Klik Latih."})});
  ...
  });$(id).addEventListener("change",function(){stop();initW();drawBoundary();$("pg-calc").textContent="Arsitektur diganti. Klik Latih."})});
